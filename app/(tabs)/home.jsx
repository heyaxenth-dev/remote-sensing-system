...
export default function HomeScreen() {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      const name = meta.full_name?.trim() || data.user.email?.split("@")[0] || "Field officer";
      setDisplayName(name);
    })();
    return () => {
      cancelled = true;
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.actionGrid}>
          {QUICK_ACTIONS.map((action) => (
            <TouchableOpacity
              key={action.key}
              style={[styles.actionTile, action.highlighted && styles.actionTileActive]}
            ...
            >
              ...
              />
              <Text style={[styles.actionLabel, action.highlighted && styles.actionLabelActive]}>
                {action.label}
              </Text>
            </TouchableOpacity>
          ))}
      ...
      <Modal visible={menuOpen} animationType="fade" transparent>
        <Pressable style={styles.modalBackdrop} onPress={() => setMenuOpen(false)}>
          <View style={styles.menuSheet}>
            ...
            <TouchableOpacity style={styles.menuItem} onPress={() => { setMenuOpen(false); router.push("/capture"); }}>
              <Text style={styles.menuItemText}>Mobile data acquisition</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.menuItem} onPress={() => { setMenuOpen(false); router.push("/offline"); }}>
              <Text style={styles.menuItemText}>Offline operation & storage</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.menuItem} onPress={() => { setMenuOpen(false); router.push("/offline/sync"); }}>
              <Text style={styles.menuItemText}>Data synchronization</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.menuItem} onPress={() => { setMenuOpen(false); router.push("/offline/map"); }}>
